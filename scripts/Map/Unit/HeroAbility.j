...
library HeroAbilityLib requires AbilityCustomOrderId
    ...
    struct HeroAbility extends array
    ...
    globals
        ...
        string  array HeroAbility_Icon
        integer array HeroAbility_Base
        integer array HeroAbility_Special
        
        integer array HeroAbility_Modify
    ...
    function GetSkillIndex takes integer id returns integer	//返回普通+A杖
        local integer i = 1
        loop
            if HeroAbility_Base[i]== id or HeroAbility_Special[i] == id then
                return i
            endif
            set i = i + 1
        exitwhen i > MaxHeroSkillsNumber
    ...
    function RegisterHeroSkill takes integer id, string StackLim, integer commonSkill, integer upgradeSkill, integer changeSkill, string sHotKey returns nothing
        if commonSkill > 0 then
            set HeroAbility_Icon[id]= GetAbilitySoundById(commonSkill, SOUND_TYPE_EFFECT_LOOPED)
        endif
        set HeroAbility_Base[id]= commonSkill
        set HeroAbility_Special[id]= upgradeSkill
        set HeroAbility_Modify[id]= changeSkill
    ...
    function InitHeroSkillsData takes nothing returns nothing
        ...
        set HeroAbility_Icon[0]= "UI\\Widgets\\Console\\Undead\\undead-inventory-slotfiller.blp"
        set i = 1 -1
        ...
        set HeroAbility_Icon[i * 4 + 4]= "ReplaceableTextures\\CommandButtons\\BTNLifeDrain.blp"
        set i = 74 -1
        ...
        set HeroAbility_Icon[i * 4 + 4]= "ReplaceableTextures\\CommandButtons\\BTNExorcism.blp"
        set i =104-1
        ...
        set HeroAbility_Icon[i * 4 + 3]= "ReplaceableTextures\\CommandButtons\\BTNManaDrain.blp"
