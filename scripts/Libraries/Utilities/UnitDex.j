...
library UnitDex uses optional WorldBounds, optional GroupUtils, Table
    /***************************************************************
    *
    ...
    */
    private module UnitDexConfig
        ...
        static method onFilter takes unit whichUnit returns boolean
    ...
    */
    ...
    function GetUnitId takes unit whichUnit returns integer
        return Table[GetHandleId(whichUnit)].integer[KEY]
    endfunction
    ...
    private module UnitDexCore
        ...
        private static method onEnter takes nothing returns boolean
            ...
            if(i == 0 and thistype.Enabled) then
                ...
                else // Counter应该是已经给出的最大分配量(不会因为释放而降低)
                    set Counter = Counter + 1
                    set t = Counter
                endif
              
                set List[t] = - 1 // 已使用
                set LastIndex = t // 最后创建的索引
                set thistype.Unit[t] = u
                set Count = Count + 1 // 已存在索引的计数
              
                // Store the index
                set Table[GetHandleId(u)].integer[KEY] = t
        ...
        static method RecycleIndex takes integer i returns nothing
        ...
        private static method onLeave takes nothing returns boolean
            ...
            if(thistype.Enabled) then
                ...
                set u = MHEvent_GetUnit()
              
                set i = GetUnitId(u)

                // If unit has been indexed then deindex it 这段是检查有效性
                if(i > 0 and i <= Counter and u == GetUnitById(i)) then
                    ...
                    // Remove entry
                    call Table[GetHandleId(u)].integer.remove(KEY)
                    //call SetUnitUserData(u, 0)
        ...
        private static method onInit takes nothing returns nothing
            ...
            call MHUnitRemoveEvent_Register(t)
            //loop
            //    set p = Player(i)
            //  
            //    // Detect "undefend"
