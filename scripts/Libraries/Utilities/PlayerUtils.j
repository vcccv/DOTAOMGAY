...
*
*       static method operator Host  takes nothing returns player
*       static method operator HostId  takes nothing returns User
...
**************************************************************/
    ...
    struct User extends array
        ...
        static method operator Host takes nothing returns player
            return thistype.first.handle
        endmethod

        static method operator HostId takes nothing returns thistype
        ...
        static method onLeave takes nothing returns boolean
            ...
            // clean up
            call ForceRemovePlayer(FORCE_PLAYING, p.handle)
            ...
            if p == thistype.first then
                set thistype.first = p.next
            endif
        ...
        private static method onInit takes nothing returns nothing
            ...
            loop
                ...
                if (GetPlayerController(p.handle) == MAP_CONTROL_USER and GetPlayerSlotState(p.handle) == PLAYER_SLOT_STATE_PLAYING) then
                    ...
                    if (thistype.first == User.NULL) then
                        set thistype.first = i
                    ...
                    set Hex[i] = OriginalHex[GetHandleId(thistype.Color[i])]
                ...
                set p.originalName=Name[i]
           
                set i = i + 1
            endloop

            set bj_isSinglePlayer = (.AmountPlaying == 1)
