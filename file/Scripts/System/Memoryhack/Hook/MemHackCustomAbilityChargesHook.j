//! nocjass
library OMemoryHackAbilityChargesHook

    #define GetChargesTable()                           LoadHashtableHandle(MemHackTable, <?=StringHash('DrawChargesAPI')?>, <?=StringHash('HashTable')?>)
    #define GetCustomUnitAbilityCharges(u,aid)          LoadInteger(GetChargesTable(), GetHandleId(u), aid)

    function InitCustomChargesData takes nothing returns nothing
        local integer i                           = 0
        local integer memSize                     = 0x140 // size = memory + 0x13C | last offset + 0x4 -> so 0x13C + 0x4 => 0x140
    ...
    function SetCustomUnitAbilityCharges takes unit u, integer aid, integer charges returns nothing
        if not HaveSavedReal( GetChargesTable( ), GetHandleId( u ), aid ) then
            call InitCustomAbilityAddressChargesHook( GetUnitAbilityData( u, aid, 1 ) )
            call SaveBoolean( GetChargesTable( ), GetHandleId( u ), aid, true )
            call SaveReal( GetChargesTable( ), GetHandleId( u ), aid, 1 )
        endif
