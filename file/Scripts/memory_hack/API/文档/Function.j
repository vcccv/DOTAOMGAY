...
// Hook库
    ...
    // 设置命令ID
    // @Tip：命令ID可自定义。设置后可以让同模板技能互不冲突。仅支持狂战士、刀阵旋风和显示三种技能
    ...
    // 获取已设置的命令ID
    // @Tip：指代 MHAbilityOrder_SetOrder 设置的命令ID。仅支持狂战士、刀阵旋风和显示三种技能
    ...
    // 设置下标状态
    // @Tip：指代 MHAbility_SetChargeCount 设置的下标。可控制下标显示或隐藏
    // 设置光环技能的下标会导致游戏崩溃
    // @param flag：true =  = 隐藏
    ...
    // 获取下标状态
    // @Tip：指代 MHAbility_SetChargeState 设置的下标状态
...
// System库
    ...
    // @param index：0~11 = 技能栏文本; 12~17 = 按钮栏文本
    ...
    // 获取自定义物编数据 (整数)
    // @Tip：该函数获取的是技能实例的物编数据，每个单位允许不同。仅支持非ui类型的数据
    // @param flag：ABILITY_LEVEL_DEF_DATA
    ...
    // 设置自定义物编数据 (整数)
    // @Tip：该函数设置的是技能实例的物编数据，每个单位允许不同。仅支持非ui类型的数据
    // @param flag：ABILITY_LEVEL_DEF_DATA
...
// 基本库
    ...
    // @param flag：伤害标志。疑似BitSet，不可自定义，取值参考表格
...
// 基本库
    ...
    // 设置jass追踪日志等级
    // @param level：JASS_LOG_LEVEL
    function MHDebug_SetJassLogLevel takes integer level returns nothing
    endfunction

    // 输出jass追踪日志到控制台
    // @param count：输出日志条数
    function MHDebug_DumpJassLog takes integer count returns nothing
    endfunction

    // 输出jass追踪日志
    // @Tip：到魔兽根目录
    function MHDebug_DumpJassLogFile takes nothing returns nothing
    endfunction

    // 允许自动生成崩溃日志
    // @Tip：该操作不可逆，会完全取代原有的崩溃处理信息
    function MHDebug_EnableCrashReport takes nothing returns nothing
    endfunction

    // 允许运行跟踪
    // @Tip：会在jass函数运行错误后 (除数为0、超出字节码限制等) 弹窗报告
    function MHDebug_EnableRunningTrace takes boolean is_enable returns nothing
    endfunction
...
// 基本库
    ...
    function MHEffect_SetAnimationEx takes effect eff, string name, string attachment returns nothing
    endfunction
...
// 基本库
    ...
    // 按键是按下的
    function MHMsg_IsKeyDown takes integer key returns booleaan
    endfunction
