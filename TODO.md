...
- [x] 基于mpqeditor重建mpq重新测试(或仅替换关键文件，例如Units，以实现不重新导入就应用的物编修改)
...
- [ ] 读书系列重做
    ...
    - [ ] 始 次 继
    - [ ] 终末
- [x] -show指令移除 定点爆破不再能被-show指令隐藏
- [x] -nh指令(禁止相同热键模式)移除
- [x] -removeiteminslot指令移除
- [x] -hkey 效果增强 如果是一个多图标技能，则可以-hkey Q W E R T连携设置额外技能
    - [x] 子技能系统
    - [x] 现在可以使用#来当做占位符，不改变某个技能的快捷键。
    - [x] -hkey和-ikey指令现在支持缩写，-hk和-ik
- [x] 现在额外信息不再只是显示技能限制，而是会显示
    - [x] 技能限制 
    - [x] 技能提示
    - [x] 技能的子技能
- [x] 消化(科多)
    ...
    - [x] 现在如果投掷充能小于1，则会被禁用
- [x] 余震
    - [x] 修正技能作用范围错误的为315，现在正确应用300作用范围。
- [x] 火之余烬
    - [x] 火之余烬现在会显示充能下标
    - [x] 火之余烬不再使用原来的4个额外图标来显示充能 
    - [x] 如果现在充能小于1，则会被禁用
- [x] 召唤巨石
    - [x] 现在只有在获得大地之灵4个技能中任意一个技能时，英雄才会持有召唤巨石技能，如果失去技能后，身上不存在任何大地之灵的技能时，召唤巨石也会被移除。
    - [x] 现在可以显示充能下标了
    - [x] 修正了错误的技能提示，现在冷却时间从35正确显示为25。
- [x] 海妖之歌
    - [x] 现在在施法效果时立即显示停止海妖之歌技能，但会进入1秒冷却时间防止快速误点。
- [x] 黑暗之门
    - [x] 现在如果黑暗之门施法者死亡，黑暗之门会正确被取消。 
- [x] X标记
    - [x] 目标单位处于无敌或隐藏状态时，X标记将不再返回该单位。 
- [x] 烈日炙烤
    - [ ] ?
- [x] 幻象法球
    - [x] 修复了最大距离为1950的错误，现在最大距离为1800与描述符合。
    - [x] 现在**灵动之翼**支持多个法球的传送，优先传送至最先创建的法球。
    - [x] 现在**灵动之翼**仅在存在幻象法球时才可用。
- [x] 灵魂汲取
    - [x] 修正了灵魂汲取在超出550范围会中断的错误，现在最大距离修正为550→600.
    - [x] 添加了施法距离和最大距离的工具提示。 
- [x] 窒息之刃
    - [x] 附带一次普通攻击效果。
    - [x] 现在不再直接造成暴击效果。
    - [x] 因为适配起来太麻烦所以现在暂时不兼容**忍术**和**醉拳**
- [x] 海象挥击
    - [x] 实装平衡改动，在非BO(关闭平衡性)模式下，海象挥击和强化图腾选择时暴击伤害减少至只有2倍，在这之前代码里有但因为bug失效了。
    - [x] 现在可以更准确触发
- [x] 生命汲取
    - [x] 神杖升级之前的施法距离从950/1050/1200 → 900
    - [x] 神杖升级后的施法距离从900/1050/1200 → 950/1050/1200
    - [x] 修正神杖升级后的工具提示施法距离错误的问题，描述从1000更正为正确的950/1050/1200
    - [x] 添加了最大距离的工具提示
- [x] 法力汲取
    - [x] 添加了最大距离的工具提示
- [x] 启明
    - [x] 现在不再需要持续施法效果合并到神杖升级中
    - [x] 神杖升级提供击中回血效果
- [x] 暂时禁用**血肉傀儡**
    - [ ] 神杖升级单独实现 
- [x] 现在高射火炮拥有神杖升级效果了
- [x] 绝大部分同类技能不再命令冲突(可以一起选)
- [x] 以下技能现在不会打断引导性施法，并且是立即发动(无视技能前后摇)
    - [x] 灵能陷阱 - 触发陷阱
    - [x] 复制 - 替换
    - [x] 幽魂 - 拉进
    - [x] 幽魂 - 散开
...
- [x] 冷却缩减系统重置现在拥有被动的冷却技能可以正确应用冷却缩减
    - [x] 手动合并带有冷却的被动技能，现在不再计入PassiveSkill系统
    - [x] 从AOre改为ANcl
    - [x] 潮汐使者
    - [x] 反击螺旋
    - [x] 勇气之霎
...
- [x] 自定义单位事件
- [x] 重写Alt显示，PlayerSettings
  - [x] HoldingALTShowsNeutralSpawnboxes
  - [x] HoldingALTShowsTowerAttackRange
...
- [x] 修正了多面板内的CD1和CD2显示错误的问题，现在正确显示2个奥义的冷却时间。
...
- [x] 修复一些物品购买时和实际价格不符合的情况下
    - [x] 科勒的匕首(禁用)价格从2150修正为2250
    - [x] 空瓶价格从650修正为700
    - [x] 血精石卷轴价格从800修正为900
- [x] 熊灵伙伴
    - [x] 现在熊灵被玩家伤害时，**召回**技能如果冷却时间低于3秒，则会被设置为3秒，而不再是原来的使技能消失。
    - [x] 修正熊灵在没有神杖升级前允许攻击距离描述内的1100而不是原来的1425。
    - [x] 修正熊灵在允许攻击距离外时，玩家难以拾取到熊灵脚下物品的问题。
    - [x] 修复了熊灵伙伴拥有协同时熊灵等级移动速度加成被协同加成覆盖的错误(熊灵移动速度330/340/350/360)，受到协同加成后移动速度变为328/336/344/352(协同等级)，现在两者正确叠加。
- [x] **静谧之鞋**现在从破碎恢复后立即更新单位移动速度。
- [x] 银月之晶在消耗后现在能正确被计入总资产
- [x] 死亡竞赛
    - [x] 现在**银月之晶**吞噬后会被新的英雄继承，如果是熊灵则会重新掉落在泉水。
    - [x] 现在**地精的贪婪**赠与的神杖会被新的英雄继承
    - [x] **阿哈利姆福佑**也会被新的英雄继承
### 游戏
- [x] 修正一个低概率导致异步的错误

## 优化
- [x] ping技能强化，并且现在可以查看队友技能。
  - [x] ping买活
- [x] 现在选择技能阶段不再会预读英雄技能，而是等待所有玩家技能选择完毕后，统一对所有技能进行预读以防止技能错位
- [x] 现在在选择技能时不再是显示技能的4级效果，而是技能的学习提示。
...
- [x] 独立TP格
  ...
  - [x] 双击对自己施法
  - [x] 从龙卷风改成通魔
  - [x] 熊灵和人的逻辑，以及死亡竞赛正确继承
      - [x] 熊灵死亡竞赛继承
      - [x] 英雄死亡竞赛继承
  - [x] 卷轴传送时与建筑最大距离从525→575
  - [x] 卷轴传送现在不再会因为目标建筑死亡而取消传送
  - [x] 工具提示
  - [x] 改回来了支援限制
- [x] 塔防按钮
  - [x] 工具提示
...
- [ ] 视野外点击要删
- [ ] debug模式下的额外安全性检查
    - [x] 注册物品时增加安全性检查，如果物品会有贩卖差价，则抛出错误
    - [ ] 合成物品后的价格检查
    - [ ] 技能描述检查
- [x] 伤害Stack
...
- [ ] 相位鞋对远程有问题
- [ ] X标记添加虚线
- [ ] 噩梦目标可以被噩梦施法者攻击(取消噩梦作为副技能，可以取消场上所有噩梦)
- [ ] 充能技能即便用完也可以显示冷却时间
- [ ] **吞噬**技能不对，学习和实际技能不是一个。
- [ ] **伤残恐惧**和**虚空**需要考虑一下改键的特殊情况
- [ ] ping到buff栏，ping到Aatk
- [ ] 英雄创建时，预添加所有属性提升类技能，以防止学技能时弹回(存疑？)需要确定是玩家技能被禁用还是添加技能导致的
- [ ] 整合rc和bo，仅bo会调整平衡性。rc仅限制组合。
- [ ] 合并Passive的slk，直接取消掉PassiveSkill系统
- [ ] 分裂箭，腐烂，开关技能的整合。
