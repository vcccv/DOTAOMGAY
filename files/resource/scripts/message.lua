...
local GetObjectName           = jass.GetObjectName
...
local function get_ability_slk(name, id)
...
local function bCanLearnSkill(order, u, level)
    local s
    local reqLevel = get_ability_slk("reqLevel", order)
    local levelSkip = get_ability_slk("levelSkip", order)
    if levelSkip == 1 then
        levelSkip = 2
    end
...
local function get_ability_str(u, ability)
    local i = get_ability_slk("levels", ability)
    local s = ''
    local s2 = ''
